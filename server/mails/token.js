...
export const tokenPurchaseWithBonusMail = options => {
...
export const tokenPurchaseMail = ({email, token, symbol, unit, price, totalPrice, discount, actualCost}) => {
  let subject = `You have purchased ${unit} SportEther ${token} Team ${unit > 1 ? 'Tokens' : 'Token'}!`;
  let mailContent = {
    title: 'Token Purchased!',
    hiddenMessage: subject,
...
export const directEtherBonusMail = ({email, bonusPercent, bonusValue, friend}) => {
...
export const indirectEtherBonusMail = ({email, bonusPercent, bonusValue, friend, friendsFriend}) => {
  let subject = `You have received ${bonusValue} Ether Bonus!`;
  let mailContent = {
